...
potted_balls = []
# 加载音效
cue_hit_sound = pygame.mixer.Sound("asserts/sounds/cue_hit.mp3")
# 定义颜色
BG = (50, 50, 50)
RED = (255, 0, 0)
WHITE = (255, 255, 255)
...
# 加载图片
table_image = pygame.image.load("asserts/images/table.jpg").convert_alpha()
...
while run:
    ...
    # 检查游戏是否结束
    if lives <= 0:
        game_over_image = pygame.image.load("asserts/images/game_over.png").convert_alpha()
        image_rect = game_over_image.get_rect(center=(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2))
        screen.blit(game_over_image, image_rect)
        game_running = False
    # 检查所有的球体是否都进了球洞
    if len(balls) == 1:
        win_image = pygame.image.load("asserts/images/win.png").convert_alpha()
        image_rect = win_image.get_rect(center=(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2))
